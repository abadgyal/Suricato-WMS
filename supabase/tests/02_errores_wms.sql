...
-- por el runner.
--
-- S-B: RPC sin p_usuario_id (autor = auth.uid()). Fijamos el JWT al admin de
-- ejemplo para que ajustar/dar_de_baja lleguen a sus validaciones (WMS002/003/007)
-- en vez de cortar por WMS009 (el control de rol se prueba en la suite del runner).
-- ============================================================================
...
select throws_like($$ select salida_evento('bb000000-0000-0000-0000-0000000000a1', 0, 'bb000000-0000-0000-0000-0000000000e1') $$,
  '%WMS002%', 'WMS002: salida con unidades = 0');
select throws_like($$ select salida_evento('bb000000-0000-0000-0000-0000000000a1', 5, 'bb000000-0000-0000-0000-0000000000e1') $$,
  '%WMS001%', 'WMS001: salida sin stock (5 > 2 disponible)');

-- --- WMS001 en crear_reserva + invariantes 3/4 -----------------------------
select throws_like($$ select crear_reserva('bb000000-0000-0000-0000-0000000000e1', 'bb000000-0000-0000-0000-0000000000a1', 5) $$,
  '%WMS001%', 'WMS001: reservar más que disponible_real (5 > 2)');
select lives_ok($$ select crear_reserva('bb000000-0000-0000-0000-0000000000e1', 'bb000000-0000-0000-0000-0000000000a1', 2) $$,
  'inv3: reservar exactamente el disponible_real se permite');
select throws_like($$ select salida_evento('bb000000-0000-0000-0000-0000000000a1', 1, 'bb000000-0000-0000-0000-0000000000e1') $$,
  '%WMS001%', 'inv4: la reserva activa bloquea la salida (disponible_real = 0)');

-- --- WMS006: producto inexistente ------------------------------------------
select throws_like($$ select salida_evento('bb000000-0000-0000-0000-0000000000ff', 1, 'bb000000-0000-0000-0000-0000000000e1') $$,
  '%WMS006%', 'WMS006: producto inexistente');

-- --- WMS005: devolución excede lo que está fuera ---------------------------
select lives_ok($$ select salida_evento('bb000000-0000-0000-0000-0000000000a2', 3, 'bb000000-0000-0000-0000-0000000000e1') $$,
  'salida de 3 unidades (prepara devolución)');
select throws_like($$ select devolver('bb000000-0000-0000-0000-0000000000e1', 'bb000000-0000-0000-0000-0000000000a2', 4, 0, 0) $$,
  '%WMS005%', 'WMS005: devolver 4 con solo 3 fuera');
select lives_ok($$ select devolver('bb000000-0000-0000-0000-0000000000e1', 'bb000000-0000-0000-0000-0000000000a2', 3, 0, 0) $$,
  'devolver exactamente lo que está fuera se permite');

-- --- WMS003 / WMS002 / WMS007 en ajustar y dar_de_baja ---------------------
select throws_like($$ select ajustar('bb000000-0000-0000-0000-0000000000a3', 'disponible', 5, '') $$,
  '%WMS003%', 'WMS003: ajuste sin motivo');
select throws_like($$ select dar_de_baja('bb000000-0000-0000-0000-0000000000a3', 1, 'disponible', '   ') $$,
  '%WMS003%', 'WMS003: baja sin motivo (solo espacios)');
select throws_like($$ select ajustar('bb000000-0000-0000-0000-0000000000a3', 'disponible', -1, 'x') $$,
  '%WMS002%', 'WMS002: ajuste a valor negativo');
select throws_like($$ select dar_de_baja('bb000000-0000-0000-0000-0000000000a3', 999, 'en_reparacion', 'x') $$,
  '%WMS007%', 'WMS007: baja de un bucket sin unidades suficientes');

-- --- WMS003 en devolver con pérdida sin motivo -----------------------------
select lives_ok($$ select salida_evento('bb000000-0000-0000-0000-0000000000a4', 3, 'bb000000-0000-0000-0000-0000000000e1') $$,
  'salida de 3 (prepara devolución con pérdida)');
select throws_like($$ select devolver('bb000000-0000-0000-0000-0000000000e1', 'bb000000-0000-0000-0000-0000000000a4', 0, 0, 1) $$,
